...
function processName(name, root, current) {
  let e;
  try {
    e = propertyNameExtension.parse(name);
  } catch (error) {
    throw new Error(`SyntaxError while parsing "${name}".\n${error.message}`);
  }
  ...
  for (const { pointer, op, value } of e.filter)
    e.contexts = e.contexts.filter((context) => {
      const left = query(pointer, root, context);
      if (value === undefined) return operations[op](left);
      if (typeof left !== typeof value) return false;
      return operations[op](left, value);
    });
...
function processValue(value, root, current) {
  ...
  let e;
  try {
    e = propertyValueExtension.parse(value);
  } catch (error) {
    throw new Error(`SyntaxError while parsing "${value}".\n${error.message}`);
  }
  if (!Array.isArray(e)) {
